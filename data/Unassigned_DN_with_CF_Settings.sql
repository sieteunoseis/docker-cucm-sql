...
FROM
  numplan np
  INNER JOIN typepatternusage tpu ON np.tkpatternusage = tpu.enum
  LEFT OUTER JOIN callforwarddynamic AS cfd ON cfd.fknumplan = np.pkid
  LEFT OUTER JOIN devicenumplanmap dnmp ON dnmp.fknumplan = np.pkid
  LEFT OUTER JOIN routepartition rpt ON rpt.pkid = np.fkroutepartition
WHERE
  tpu.NAME = 'Device'
